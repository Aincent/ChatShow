...
void conn_tcpprotocol(int serverid,uint32_t ip,int port,void* msg,int len,uint64_t userid,uint32_t handlerid,int protocol_type)
{
	switch(protocol_type)
	{
	case NETPROTOCOL_TYPE_TCP4:
		tcp_cennectserver(serverid,ip,port,msg,len,userid,handlerid,&_g_protocol[1],tcpprotocol_create_tcp4,tcpprotocol_destory);
		break;
	case NETPROTOCOL_TYPE_TCP12:
		tcp_cennectserver(serverid,ip,port,msg,len,userid,handlerid,&_g_protocol[2],tcpprotocol_create_tcp12,tcpprotocol_destory);
		break;
	}
}
